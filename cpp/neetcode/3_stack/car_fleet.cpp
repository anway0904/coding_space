...
mile of the ith car and speed[i] is the speed of the ith car in miles per hour.
    ...
    1. A car cannot pass another car, but it can catch up and then travel next to it at the speed of the slower car.
    2. A car fleet is a car or cars driving next to each other. The speed of the car fleet is the minimum speed 
       of any car in the fleet.
    3. If a car catches up to a car fleet at the mile target, it will still be considered as part of the car fleet.
...
public:
    int carFleet(int target, std::vector<int>& position, std::vector<int>& speed) {
        std::stack<double> timeToTarget;
        std::vector<std::pair<float, float>> sortedPosSpeed;
        ...
        for (int i=0 ; i<sortedPosSpeed.size() ; i++) {
            double time = (target - sortedPosSpeed[i].first)/sortedPosSpeed[i].second;
            if(!timeToTarget.empty() && time<=timeToTarget.top()) continue;
